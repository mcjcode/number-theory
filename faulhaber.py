...
def faulhaber_polynomial(p):
    ...
    """
    n = sympy.symbols('n')
    Fp = sum(binomial(p+1, r)*bernoulli(r)*n**(p+1-r)
...
def generate_polynomials(modulus):
    ...
    retval += 'cs= ['
    for p in range(51):
        binom = sympy.functions.combinatorial.numbers.binomial
        bern = sympy.functions.combinatorial.numbers.bernoulli
        Fp = sum(binom(p+1, r) * bern(r) * n**(p+1-r) for r in range(p+1))
        Fp = Fp / (p+1) * math.factorial(p+1)
    ...
    retval += ('    return (retval*pow(math.factorial(p+1), -1, modulus))'
               '%modulus\n')
    return retval
