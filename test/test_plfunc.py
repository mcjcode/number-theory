...
class PLFuncTest(unittest.TestCase):
    ...
    def test_prune_constant_length(self):
        ...
        self.assertEqual(len(
            g.xs), 2,
            'pruned constant function should only have 2 nodes, has %s'
            % len(g.xs))
    ...
    def test_prune_linear_length(self):
        ...
        self.assertEqual(len(
            g.xs), 2,
            'pruned linear function should only have 2 nodes, has %s'
            % len(g.xs))
    ...
    def test_prune_quadratic_length(self):
        ...
        self.assertEqual(len(
            g.xs), N+1,
            'pruned quadratic function should have as many nodes as original,'
            ' has %s' % len(g.xs))
