...
class SLVTest(unittest.TestCase):
    ...
    def test_dim2_L1(self):
        for _ in range(100):
            ...
            for _ in range(20):
                ...
                A = A.dot(B)
            self.assertEqual(
                abs(A[0, 0]*A[1, 1]-A[1, 0]*A[0, 1]), 1,
                f'abs(det({A})) should equal 1')
            u = A[:, 0]
            v = A[:, 1]
            u, v = lagrangeL1(u, v)
            self.assertEqual(sum(map(
                abs, u)), 1,
                f'{A[:,0]} {A[:,1]}. smallest vector {u}, {v}'
                ' in lattice should have norm 1')
    ...
    def test_dim2_L2(self):
        for _ in range(100):
            ...
            self.assertEqual(A[0, 0]*A[1, 1]-A[1, 0]*A[0, 1],
                             1, f'det({A}) should equal 1')
            u = A[:, 0]
            v = A[:, 1]
            u, v = lagrange(u, v)
            self.assertEqual(
                sum(map(abs, u)), 1,
                f'smallest vector {u} in lattice should have norm 1.')
    ...
    def test_minkowskiL1(self):
        for _ in range(100):
            def norm(u):
                return sum(map(abs, u))
            u = np.random.randint(-100, +100, size=(2,))
            v = np.random.randint(-100, +100, size=(2,))
            u, v = lagrangeL1(u, v)
            volume = abs(u[0]*v[1] - u[1]*v[0])
            self.assertTrue(norm(u) <= math.sqrt(2*volume),
                            norm(u)/math.sqrt(2*volume))
