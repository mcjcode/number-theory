...
def printV(V, tag=''):
    if debug:
        fmtstr = '%' + f'{fwidth}' + 'd'
        print(' '.join([fmtstr % (i,)
              for i in V[-column_limit:]])+(' :%s'%(tag,)))


def printS(V, S, tag=''):
    if debug:
        fmtstr = '%' + f'{fwidth}' + 'd'
        print(' '.join([fmtstr % (S[i],)
              for i in V[-column_limit:]])+(' :%s'%(tag,)))
...
def printRuler(V, tag=''):
    if debug:
        print(
            '-'.join(['-'*fwidth for i in V[-column_limit:]])+(' :%s'%(tag,)))
    ...
    for v in V:
        ...
        while pb <= sqrtv:  # ub: #sqrtn:
            ...
            if S0[pb] > S0[pb-1]:  # pb is prime
                ...
                # print('%d primes < %d//%d=%d' % (S0[v//pb],v,pb,v//pb))
                # print('primes in [%d,%d]=%d'
                #       % (pb,v//pb,S0[v//pb]-(S0[pb]-1)))
                S[v] += S0[v//pb]  # -(S0[pb]-1)
            pb += 1
